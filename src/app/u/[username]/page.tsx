...
function load(username: string) { return getDb().prepare("SELECT p.*, u.created_at FROM profiles p JOIN users u ON u.id=p.user_id WHERE p.username = ? AND p.is_public = 1").get(username.replace(/^@/, "")) as { user_id: string; username: string; display_name: string; bio: string; avatar_url: string; links_json: string; created_at: number } | undefined; }
export async function generateMetadata({ params }: P): Promise<Metadata> { const { username } = await params; const p = load(username); if (!p) notFound(); return { title: `${p.display_name || p.username} (@${p.username})`, description: p.bio || undefined }; }
export default async function Portfolio({ params }: P) {
  const { username } = await params; const p = load(username); if (!p) notFound();
  const works = docs.publicByUser(p.user_id); const links = JSON.parse(p.links_json || "{}") as Record<string, string>;
    ...
    <main className="min-h-dvh">
      ...
      <section className="mx-auto max-w-4xl px-4 pt-6 pb-24">
        <div className="flex items-start gap-5">{p.avatar_url ? /* eslint-disable-next-line @next/next/no-img-element */ <img src={p.avatar_url} alt="" className="h-20 w-20 rounded-full object-cover border hairline" /> : <div className="h-20 w-20 rounded-full bg-blue flex items-center justify-center serif text-3xl text-[#f1ebdd]">{(p.display_name || p.username)[0].toUpperCase()}</div>}
          <div><h1 className="text-4xl">{p.display_name || p.username}</h1><div className="text-ink-3 text-sm">@{p.username} · writing since {new Date(p.created_at).getFullYear()}</div>{p.bio && <p className="mt-3 text-ink-2 max-w-xl leading-relaxed">{p.bio}</p>}<div className="mt-3 flex flex-wrap gap-2">{Object.entries(links).filter(([, v]) => v).map(([k, v]) => <a key={k} href={v} target="_blank" rel="noreferrer" className="chip">{k}</a>)}</div></div></div>
